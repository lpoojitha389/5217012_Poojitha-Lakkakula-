...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    int i = unique_count - 1; // Start from the end of unique scores

    for (int j = 0; j < player_count; j++) {
        int score = player[j];

        // Move up the leaderboard while player's score is greater or equal
        while (i >= 0 && score >= unique[i]) {
            i--;
        }

        result[j] = i + 2; // +2 because rank is i+1, and i was decremented
    }
